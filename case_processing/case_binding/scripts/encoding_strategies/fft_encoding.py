...
class FFTEncoding(object):
    ...
    def __apply_FFT(self, index):
        ...
        yf = fft(row)
        np.linspace(0.0, 1.0 / (2.0 * T), self.stop_value // 2)
    ...
    def encoding_dataset(self):
        ...
        data_process_FFT = Parallel(n_jobs=6, require="sharedmem")(
            delayed(self.__apply_FFT)(i) for i in range(len(self.dataset))
        )
